...
You have three read-only tools: read_file, list_dir and search. They are the only way you touch the workspace, and they cannot change anything or run anything — there is no command execution, no write, and no network.
...
correctness, concurrency, error-handling, security, api-misuse, performance, tests, consistency.
...
function configPath() {
  return join(process.env.DSH_HOME ?? process.cwd(), 'code-review', 'config.json')
}
...
function parseInvocation(rawInput) {
  ...
  for (;;) {
    ...
    if (match === null) break
    const token = match[1]
    if (scope === undefined && (token === 'full' || token === 'session')) {
      scope = token
      rest = rest.slice(match[0].length)
      continue
    }
    const pair = /^([A-Za-z]+)=(.+)$/.exec(token)
    if (pair !== null && ARG_KEYS.has(pair[1])) {
...
async function collectGitChanges(ctx, cwd, settings, base, scope, touchedFor, signal, log) {
  ...
  const changed = sessionOnly ? changedAll.filter(fromSession) : changedAll
  const untracked = (sessionOnly ? untrackedAll.filter(fromSession) : untrackedAll)

  if (changed.length === 0 && untracked.length === 0) {
    return sessionOnly
      ? { failure: 'session-clean', touched: touchedPaths, files: changedAll.length + untrackedAll.length }
      : { failure: 'clean' }
  }
  ...
  const filesTotal = changedAll.length + untrackedAll.length
  if (reviewed === 0) return { failure: 'all-skipped', skipped, files, filesTotal }
...
const IGNORED_DIRS = new Set([
  ...
  'out', 'coverage', '__pycache__', '.venv', 'venv', '.next', '.nuxt', '.cache',
  '.idea', '.vscode', '.gradle', '.terraform', 'bower_components',
])

const READER_TOOLS = [
  {
    name: 'read_file',
    description: 'Read a UTF-8 text file from the workspace. The reply starts with a header naming the path and the line range; the content follows without line-number prefixes. Read only what settles a specific question.',
    parameters: {
      type: 'object',
      properties: {
...
function readFileTool(args, root, budget) {
...
function listDirTool(args, root, budget, signal) {
  const target = resolveInside(root, args.path)
  if (target === undefined) return { text: `refused: "${args.path}" is outside the workspace`, isError: true }
  let entries
  try {
  ...
  const visible = entries.filter(entry => !IGNORED_DIRS.has(entry.name))
  ...
  budget.bytes += shown.length
  return {
    text: `[${relativePath(root, target)} — ${visible.length} entries${visible.length > rows.length ? ', truncated' : ''}]\n${shown}`,
    extraLines: visible.map(entry => entry.name),
  }
}

function searchTool(args, root, budget, signal, maxSearchResults) {
  ...
  while (stack.length > 0 && found.length < maxResults && scanned < 3000 && chars < room) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (!IGNORED_DIRS.has(entry.name) && !entry.name.startsWith('.')) stack.push(full)
        continue
      }
...
function runReaderTool(call, root, budget, signal, settings) {
  let args
  try {
  ...
  if (call.name === 'read_file') return readFileTool(args, root, budget)
  if (call.name === 'list_dir') return listDirTool(args, root, budget, signal)
  if (call.name === 'search') return searchTool(args, root, budget, signal, settings.maxSearchResults)
...
async function reviewWithReader({ ctx, route, settings, prompt, root, signal, deadlineAt, log }) {
  ...
  for (let step = 0; ; step += 1) {
    ...
    for (const call of answer.calls) {
      ...
      budget.calls += 1
      const result = runReaderTool(call, root, budget, signal, settings)
...
- workspace: ${cwd}
- changed files: ${stats.files} (+${stats.added} / -${stats.deleted} lines), reviewed here: ${stats.reviewed}${skipped}${scope}
...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.commands.register({
    ...
    input: { hint: '[full|session] [provider=<id>] [model=<id>] [focus message]' },
    handler: async ({ agent, rawInput, signal }) => {
      ...
      if (sourceMode !== 'session') {
        ...
        if (collected.failure !== undefined) {
          ...
          if (collected.failure === 'all-skipped') {
            return {
              kind: 'error',
              text: `code-review: ${collected.files} changed file(s) against ${gitRev}, but every one was skipped (binary, oversized, or over the diff budget) — raise maxFiles/maxDiffChars and run /review again.`,
            }
          }
      ...
      if (collected.reviewed === 0) {
        const why = collected.files === 0 ? 'no file changed' : 'every changed file was skipped (binary, oversized, or over the diff budget)'
